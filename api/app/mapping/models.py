...
from sqlalchemy import String, Text, DateTime, Index, text
...
class PiiMapping(Base):
    ...
    __table_args__ = (
        Index("uix_mapping_tenant_project", "tenant_id", "project_id", "context_id", "context_type", "token", unique=True, postgresql_where=text("tenant_id IS NOT NULL"), sqlite_where=text("tenant_id IS NOT NULL")),
        Index("uix_mapping_self_hosted_project", "project_id", "context_id", "context_type", "token", unique=True, postgresql_where=text("tenant_id IS NULL"), sqlite_where=text("tenant_id IS NULL")),
    ...
    project_id: Mapped[str] = mapped_column(String(100), default="default", nullable=False)
