...
export function ApiKeysPage({ isAdmin }: ApiKeysPageProps) {
  ...
  function load() {
    api.listApiKeys()
      .then(setKeys)
      .catch(() => setError("Impossibile caricare le chiavi."))
  ...
  async function handleCreate(e: FormEvent) {
    ...
    try {
      ...
      setError("Impossibile creare la chiave.");
    }
  }
  ...
  async function handleRevoke(id: string) {
    try {
      ...
      setError("Impossibile revocare la chiave.");
    }
  }
              ...
              {keys.map((k) => (
                <tr key={k.id} className="border-b border-slate-700/50">
                  ...
                  <td className="px-4 py-3 text-slate-400">
                    {k.last_used_at ? new Date(k.last_used_at).toLocaleString("it-IT") : "mai"}
                  </td>
                  <td className="px-4 py-3">
                    {k.active && (
                      <button
      ...
      {showModal && !createdKey && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-sm">
            <h2 className="font-semibold mb-4">Crea nuova API Key</h2>
      ...
      {createdKey && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-md">
            <h2 className="font-semibold mb-2">Chiave creata</h2>
            <p className="text-slate-400 text-sm mb-4">Copiala ora. Non verra mostrata di nuovo.</p>
            ...
            <button
              onClick={() => { setCreatedKey(null); setShowModal(false); }}
              className="mt-4 w-full bg-indigo-600 hover:bg-indigo-700 rounded px-3 py-2 text-sm"
            >
              Chiudi
            </button>
          </div>
